...
def clear_piece(pos: Board, sq: Squares) -> None:
	...
	pos.bitboards[pce.value].clear_bit(sq)
	pos.occupancies[col.value].clear_bit(sq)
	pos.occupancies[Colour.BOTH.value].clear_bit(sq)
	return

def add_piece(pos: Board, sq: Squares, pce: Piece) -> None:
	col = piece_col[pce]

	pos.pieces[sq.value] = pce
	pos.piece_num[pce.value] += 1

	pos.bitboards[pce.value].set_bit(sq)
	pos.occupancies[col.value].set_bit(sq)
	pos.occupancies[Colour.BOTH.value].set_bit(sq)
	return

# "from" is a Python keyword, so we renamed it here
def move_piece(pos: Board, from_sq: Squares, to_sq: Squares) -> None:
	pce = pos.pieces[from_sq.value]
	col = piece_col[pce]

	pos.pieces[from_sq.value] = Piece.EMPTY
	pos.bitboards[pce.value].clear_bit(from_sq)
	pos.occupancies[col.value].clear_bit(from_sq)
	pos.occupancies[Colour.BOTH.value].clear_bit(from_sq)

	pos.pieces[to_sq.value] = Piece.EMPTY
	pos.bitboards[pce.value].set_bit(to_sq)
	pos.occupancies[col.value].set_bit(to_sq)
	pos.occupancies[Colour.BOTH.value].set_bit(to_sq)
	return
...
def take_move(pos: Board):
    ...
    if move.enpassant:
        ...
        if pos.side == Colour.WHITE:
            add_piece(pos, Squares(to_square.value + 8), Piece.bP)
        else:
            add_piece(pos, Squares(to_square.value - 8), Piece.wP)
            
    elif move.castling:
        # Move the castling rook back
        if to_square == Squares.c1:
            move_piece(pos, Squares.d1, Squares.a1)
        elif to_square == Squares.c8:
            move_piece(pos, Squares.d8, Squares.a8)
        elif to_square == Squares.g1:
            move_piece(pos, Squares.f1, Squares.h1)
        elif to_square == Squares.g8:
            move_piece(pos, Squares.f8, Squares.h8)

    move_piece(pos, to_square, from_square)
    ...
    if captured != Piece.EMPTY and not move.enpassant:
        add_piece(pos, to_square, captured)

    # Undo promotion
    if move.promoted != Piece.EMPTY:
        clear_piece(pos, from_square)
        add_piece(pos, from_square, Piece.wP if piece_col[move.promoted] == Colour.WHITE else Piece.bP)

def make_move(pos: Board, move: Move) -> bool:
    from_square = move.source
    to_square = move.target
    side = pos.side
    ...
    if move.enpassant:
        ...
        if side == Colour.WHITE:
            clear_piece(pos, Squares(to_square.value + 8))
        else:
            clear_piece(pos, Squares(to_square.value - 8))
        ...
        if to_square == Squares.c1:
            move_piece(pos, Squares.a1, Squares.d1)
        elif to_square == Squares.c8:
            move_piece(pos, Squares.a8, Squares.d8)
        elif to_square == Squares.g1:
            move_piece(pos, Squares.h1, Squares.f1)
        elif to_square == Squares.g8:
            move_piece(pos, Squares.h8, Squares.f8)
    ...
    move_piece(pos, from_square, to_square)
