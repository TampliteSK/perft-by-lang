...
pawn_attacks = [[0 for _ in range(64)] for _ in range(2)]  # [side][square]
...
def get_bishop_attacks(sq: Squares, occupancy: Bitboard) -> Bitboard:
    blockers = occupancy & bishop_masks[sq.value] * bishop_magic_numbers[sq.value]
    index = blockers >> (64 - bishop_relevant_bits[sq.value])
    if index >= len(bishop_attacks[sq.value]):
        print(f"Debug: index={index}, sq={sq}, mask={int(bishop_masks[sq.value])}, occ_in={int(occupancy)}, magic={bishop_magic_numbers[sq.value]}, relbits={bishop_relevant_bits[sq.value]}")
        print_bitboard(occupancy)
        mask = bishop_masks[sq.value]
        print_bitboard(mask)
        raise ValueError(f"Invalid magic index {index} for bishop_attacks[{sq.value}]")
    return bishop_attacks[sq.value][index]

# get rook attacks
def get_rook_attacks(sq: Squares, occupancy: Bitboard) -> Bitboard:
    blockers = occupancy & rook_masks[sq.value] * rook_magic_numbers[sq.value]
    index = blockers >> (64 - rook_relevant_bits[sq.value])
    if index >= len(rook_attacks[sq.value]):
        print(f"Debug: index={index}, sq={sq}, mask={int(rook_masks[sq.value])}, occ_in={int(occupancy)}, magic={rook_magic_numbers[sq.value]}, relbits={rook_relevant_bits[sq.value]}")
        print_bitboard(occupancy)
        mask = rook_masks[sq.value]
        print_bitboard(mask)
        raise ValueError(f"Invalid magic index {index} for bishop_attacks[{sq.value}]")
    return rook_attacks[sq.value][index]
